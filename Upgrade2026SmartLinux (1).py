...
class SlitherMonitor:
    """
    Monitors a Solidity project with Slither.
    Runs slither on a target path and parses detector output into structured results.
    """

    SEVERITY_ORDER = {'High': 0, 'Medium': 1, 'Low': 2, 'Informational': 3, 'Optimization': 4}

    @staticmethod
    def run_analysis(target_path: str, extra_args: str = "") -> Dict:
        """
        Run slither on target_path and return parsed results dict:
          {
            'success': bool,
            'raw': str,
            'findings': [{'id': int, 'severity': str, 'check': str, 'description': str}],
            'summary': {'High': int, 'Medium': int, 'Low': int, 'Informational': int, 'Optimization': int},
            'error': str
          }
        """
        result = {
            ...
            'raw': '',
            'findings': [],
            'summary': {k: 0 for k in SlitherMonitor.SEVERITY_ORDER},
            'error': ''
        }
        ...
        cmd = f"slither {target_path} --json -"
        if extra_args.strip():
        ...
        try:
            ...
            # Try to parse JSON output from slither --json -
            ...
            if json_text:
                try:
                    ...
                    for i, det in enumerate(detectors):
                        sev = det.get('impact', 'Informational').capitalize()
                        check = det.get('check', '?')
                        desc = det.get('description', '').strip().replace('\n', ' ')
                        findings.append({
                            'id': i + 1,
                            'severity': sev,
                            'check': check,
                        ...
                        if sev in result['summary']:
                            ...
                            result['summary']['Informational'] += 1

                    # Sort by severity
                    findings.sort(key=lambda x: SlitherMonitor.SEVERITY_ORDER.get(x['severity'], 99))
                    result['findings'] = findings
                    result['success'] = data.get('success', True)
                except json.JSONDecodeError:
                    # Fallback: text mode
                    result['findings'] = SlitherMonitor._parse_text_output(raw_output, result['summary'])
            ...
            if proc.returncode not in (0, 1) and not result['findings']:
                result['error'] = raw_output[:500]

        except subprocess.TimeoutExpired:
            result['error'] = "Slither timed out after 120 seconds"
        except Exception as e:
            result['error'] = str(e)
    ...
    @staticmethod
    def _parse_text_output(text: str, summary: Dict) -> List:
        """Fallback: parse plain-text slither output for severity lines."""
        import re
        findings = []
        severity_pattern = re.compile(
            r'(High|Medium|Low|Informational|Optimization)[\s:]+(.*)',
            re.IGNORECASE
        )
        seen = set()
        idx = 1
...
class Upgrade2026App:
    ...
    def __init__(self, root):
        ...
        # Slither monitor state
        self._slither_thread = None
        self._slither_stop_flag = threading.Event()
        self.slither_path_var = None
        self.slither_args_var = None
    ...
    def build_ui(self):
        ...
        # Notebook (tabs)
        style = ttk.Style()
        style.configure('Dark.TNotebook', background=self.COLORS['bg_primary'], borderwidth=0)
        ...
        # Tab 1 — Tools manager
        tools_tab = tk.Frame(self.notebook, bg=self.COLORS['bg_primary'])
        self.notebook.add(tools_tab, text='⚙ Tools Manager')
        self.build_tools_table(tools_tab)
        self.build_log_area(tools_tab)
        self.build_controls(tools_tab)
        self.build_status_indicator(tools_tab)

        # Tab 2 — Slither Monitor
        monitor_tab = tk.Frame(self.notebook, bg=self.COLORS['bg_primary'])
        self.notebook.add(monitor_tab, text='🔍 Slither Monitor')
        self.build_slither_monitor_tab(monitor_tab)
    
    def build_slither_monitor_tab(self, parent):
        """Build the Slither Monitor tab UI."""
        C = self.COLORS

        # ── Top controls ──────────────────────────────────────────────
        ctrl_frame = tk.Frame(parent, bg=C['bg_primary'])
        ctrl_frame.pack(fill=tk.X, pady=8)

        tk.Label(ctrl_frame, text="Target path:",
                 font=('Ubuntu', 10), bg=C['bg_primary'], fg=C['text_secondary']
                 ).pack(side=tk.LEFT, padx=(0, 6))

        self.slither_path_var = tk.StringVar(value=os.path.expanduser('~/project'))
        path_entry = tk.Entry(ctrl_frame, textvariable=self.slither_path_var,
                              font=('Courier New', 10),
                              bg=C['bg_tertiary'], fg=C['text_primary'],
                              insertbackground=C['accent_primary'],
                              relief=tk.FLAT, width=50)
        path_entry.pack(side=tk.LEFT, padx=4, ipady=4)

        def browse_path():
            from tkinter import filedialog
            chosen = filedialog.askdirectory(title="Select Solidity project folder")
            if chosen:
                self.slither_path_var.set(chosen)

        tk.Button(ctrl_frame, text="📂 Browse", command=browse_path,
                  font=('Ubuntu', 9), bg=C['bg_tertiary'], fg=C['text_primary'],
                  relief=tk.FLAT, padx=8, pady=4, cursor='hand2').pack(side=tk.LEFT, padx=4)

        tk.Label(ctrl_frame, text="Extra args:",
                 font=('Ubuntu', 10), bg=C['bg_primary'], fg=C['text_secondary']
                 ).pack(side=tk.LEFT, padx=(12, 4))

        self.slither_args_var = tk.StringVar(value="")
        tk.Entry(ctrl_frame, textvariable=self.slither_args_var,
                 font=('Courier New', 10),
                 bg=C['bg_tertiary'], fg=C['text_primary'],
                 insertbackground=C['accent_primary'],
                 relief=tk.FLAT, width=20).pack(side=tk.LEFT, padx=4, ipady=4)

        self.slither_run_btn = tk.Button(
            ctrl_frame, text="▶ Run Slither", command=self.run_slither_monitor,
            font=('Ubuntu', 10, 'bold'), bg=C['accent_primary'], fg=C['bg_primary'],
            relief=tk.FLAT, padx=14, pady=6, cursor='hand2'
        )
        self.slither_run_btn.pack(side=tk.LEFT, padx=10)

        self.slither_stop_btn = tk.Button(
            ctrl_frame, text="■ Stop", command=self.stop_slither_monitor,
            font=('Ubuntu', 10, 'bold'), bg=C['accent_error'], fg=C['bg_primary'],
            relief=tk.FLAT, padx=10, pady=6, cursor='hand2', state=tk.DISABLED
        )
        self.slither_stop_btn.pack(side=tk.LEFT, padx=2)

        # ── Summary bar ───────────────────────────────────────────────
        sum_frame = tk.Frame(parent, bg=C['bg_secondary'])
        sum_frame.pack(fill=tk.X, pady=3)

        self.slither_summary_labels = {}
        sev_colors = {
            'High': C['accent_error'],
            'Medium': C['accent_warning'],
            'Low': '#ffff00',
            'Informational': C['accent_primary'],
            'Optimization': C['accent_success'],
        }
        for sev, color in sev_colors.items():
            tk.Label(sum_frame, text=f"{sev}:", font=('Ubuntu', 9, 'bold'),
                     bg=C['bg_secondary'], fg=color, padx=8, pady=4).pack(side=tk.LEFT)
            lbl = tk.Label(sum_frame, text="—", font=('Ubuntu', 9, 'bold'),
                           bg=C['bg_secondary'], fg=color, padx=0, pady=4)
            lbl.pack(side=tk.LEFT, padx=(0, 14))
            self.slither_summary_labels[sev] = lbl

        self.slither_status_lbl = tk.Label(
            sum_frame, text="Idle", font=('Ubuntu', 9),
            bg=C['bg_secondary'], fg=C['text_secondary'], padx=14
        )
        self.slither_status_lbl.pack(side=tk.RIGHT)

        # ── Findings table ────────────────────────────────────────────
        table_outer = tk.Frame(parent, bg=C['bg_secondary'],
                               highlightthickness=1, highlightbackground=C['border'])
        table_outer.pack(fill=tk.BOTH, expand=True, pady=5)

        col_hdr = tk.Frame(table_outer, bg=C['bg_tertiary'])
        col_hdr.pack(fill=tk.X)
        for hdr, w in [('#', 4), ('Severity', 12), ('Check', 22), ('Description', 80)]:
            tk.Label(col_hdr, text=hdr, width=w, font=('Ubuntu', 9, 'bold'),
                     bg=C['bg_tertiary'], fg=C['accent_primary'],
                     padx=8, pady=6, anchor='w').pack(side=tk.LEFT)

        list_frame = tk.Frame(table_outer, bg=C['bg_secondary'])
        list_frame.pack(fill=tk.BOTH, expand=True)

        self.slither_canvas = tk.Canvas(list_frame, bg=C['bg_secondary'],
                                        highlightthickness=0)
        sb = ttk.Scrollbar(list_frame, orient=tk.VERTICAL,
                           command=self.slither_canvas.yview)
        self.slither_canvas.configure(yscrollcommand=sb.set)
        sb.pack(side=tk.RIGHT, fill=tk.Y)
        self.slither_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        self.slither_findings_frame = tk.Frame(self.slither_canvas, bg=C['bg_secondary'])
        self.slither_canvas_window = self.slither_canvas.create_window(
            (0, 0), window=self.slither_findings_frame, anchor='nw'
        )

        def _on_frame_configure(e):
            self.slither_canvas.configure(
                scrollregion=self.slither_canvas.bbox('all'))

        def _on_canvas_configure(e):
            self.slither_canvas.itemconfig(
                self.slither_canvas_window, width=e.width)

        self.slither_findings_frame.bind('<Configure>', _on_frame_configure)
        self.slither_canvas.bind('<Configure>', _on_canvas_configure)

        # ── Raw output area ───────────────────────────────────────────
        raw_label = tk.Label(parent, text="Raw Output",
                             font=('Ubuntu', 9, 'bold'),
                             bg=C['bg_primary'], fg=C['text_secondary'], pady=3)
        raw_label.pack(anchor='w')

        raw_frame = tk.Frame(parent, bg=C['bg_secondary'],
                             highlightthickness=1, highlightbackground=C['border'])
        raw_frame.pack(fill=tk.X, pady=3)

        self.slither_raw_box = scrolledtext.ScrolledText(
            raw_frame, height=6, font=('Courier New', 8),
            bg=C['bg_tertiary'], fg=C['text_secondary'],
            insertbackground=C['accent_primary'], relief=tk.FLAT, padx=8, pady=6
        )
        self.slither_raw_box.pack(fill=tk.X)

        # Internal state
        self._slither_thread = None
        self._slither_stop_flag = threading.Event()

    def run_slither_monitor(self):
        """Start Slither analysis in background thread."""
        if self._slither_thread and self._slither_thread.is_alive():
            return

        target = self.slither_path_var.get().strip()
        extra  = self.slither_args_var.get().strip()

        if not target:
            messagebox.showwarning("Slither Monitor", "Please enter a target path.")
            return

        self._slither_stop_flag.clear()
        self.slither_run_btn.config(state=tk.DISABLED)
        self.slither_stop_btn.config(state=tk.NORMAL)
        self.slither_status_lbl.config(text="⏳ Running...",
                                       fg=self.COLORS['accent_warning'])
        self._clear_findings()

        def _worker():
            result = SlitherMonitor.run_analysis(target, extra)
            if not self._slither_stop_flag.is_set():
                self.root.after(0, lambda: self._display_slither_results(result))

        self._slither_thread = threading.Thread(target=_worker, daemon=True)
        self._slither_thread.start()

    def stop_slither_monitor(self):
        """Signal stop and re-enable run button."""
        self._slither_stop_flag.set()
        self.slither_run_btn.config(state=tk.NORMAL)
        self.slither_stop_btn.config(state=tk.DISABLED)
        self.slither_status_lbl.config(text="Stopped", fg=self.COLORS['accent_error'])

    def _clear_findings(self):
        """Clear previous findings from the table."""
        for widget in self.slither_findings_frame.winfo_children():
            widget.destroy()
        self.slither_raw_box.delete('1.0', tk.END)
        for lbl in self.slither_summary_labels.values():
            lbl.config(text="—")

    def _display_slither_results(self, result: Dict):
        """Render findings into the table."""
        C = self.COLORS
        sev_colors = {
            'High': C['accent_error'],
            'Medium': C['accent_warning'],
        ...
        for sev, count in result['summary'].items():
            if sev in self.slither_summary_labels:
                self.slither_summary_labels[sev].config(text=str(count))

        # Status
        if result['error']:
            self.slither_status_lbl.config(
                text=f"✗ Error: {result['error'][:60]}", fg=C['accent_error'])
        elif not result['findings']:
            self.slither_status_lbl.config(text="✓ No issues found", fg=C['accent_success'])
        else:
            total = len(result['findings'])
            self.slither_status_lbl.config(
                text=f"✓ Done — {total} finding(s)", fg=C['accent_warning'])

        # Populate findings rows
        if result['findings']:
            for finding in result['findings']:
                sev = finding['severity']
                color = sev_colors.get(sev, C['text_primary'])
                bg = C['bg_secondary'] if finding['id'] % 2 == 0 else C['bg_tertiary']

                row = tk.Frame(self.slither_findings_frame, bg=bg)
                ...
                for val, w in [
                    ...
                    tk.Label(row, text=val, width=w, font=('Courier New', 8),
                             bg=bg, fg=color,
                             padx=8, pady=4, anchor='w',
                             wraplength=500 if w == 80 else 0,
                             justify=tk.LEFT).pack(side=tk.LEFT, fill=tk.X)
        else:
            tk.Label(self.slither_findings_frame,
                     text="No findings." if not result['error'] else result['error'],
                     font=('Ubuntu', 10), bg=C['bg_secondary'],
                     fg=C['text_secondary'], pady=20).pack()

        # Raw output
        self.slither_raw_box.insert(tk.END, result['raw'])
        self.slither_raw_box.see(tk.END)
